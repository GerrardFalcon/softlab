...
def run_process(process: Process, scheduler: Optional[Scheduler] = None, 
    ...
    while process.has_more():
        if verbose:
            verb = 'Running [{}] {}'.format(ticks, '.' * ((ticks % 8) + 1))
            if len(verb) < verb_len:
                verb += ' ' * (verb_len - len(verb))
            else:
                verb_len = len(verb)
            print(verb, end='\r')
        ...
        if not process.join(scheduler):
            if verbose:
                print(f'Failed to join in tick {ticks}')
            rst = False
            break
        ticks += 1
    used = time.perf_counter() - t0
    if verbose:
        verb = 'Succeed' if rst else 'Failed'
        if len(verb) < verb_len:
            verb += ' ' * (verb_len - len(verb))
        print(verb)
...
if __name__ == '__main__':
    ...
    class SaluteProcess(Process):
        ...
        def commit(self, scheduler: Scheduler) -> bool:
            if not self._finished and len(self._point) == 0:
                point = scheduler.acquire_point()
                rst = scheduler.commit_action(Action(
                    '', point, func=self.salute
                ))
                if rst:
                    self._point = point
                return rst
